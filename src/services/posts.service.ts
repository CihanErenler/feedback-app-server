...
const buildOrderBy = (sort?: string) => {
  if (sort === "old_to_new") return [{ pinned: "desc" as const }, { created_at: "asc" as const }];
  if (sort === "new_to_old") return [{ pinned: "desc" as const }, { created_at: "desc" as const }];
  return [{ pinned: "desc" as const }, { vote_count: "desc" as const }];
};

export const getAllPosts = async (filters: PostFilters): Promise<PostWithAuthor[]> => {
...
export const updatePost = async (id: string, data: Partial<Post>): Promise<PostWithAuthor | null> => {
...
export const votePost = async (
  ...
  const [, post] = await prisma.$transaction([
    ...
    prisma.post.update({ where: { id: postId }, data: { vote_count: { increment: 1 } } }),
  ]);
